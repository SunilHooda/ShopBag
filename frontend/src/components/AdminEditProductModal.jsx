...
import uploadImage from "../helpers/uploadImageToCloud";
import EnlargeImage from "./EnlargeImage";
import { MdDelete } from "react-icons/md";
import { toast } from "react-toastify";
import apiCalls from "../helpers/apiCalls";

const AdminEditProductModal = ({ onClose, productData, fetchData }) => {
  ...
  const [openEnlargeImage, setOpenEnlargeImage] = useState(false);
  ...
  const handleSubmitProduct = async (e) => {
    ...
    const dataResponse = await fetch(apiCalls.updateProduct.url, {
      method: apiCalls.updateProduct.method,
      credentials: "include",
      headers: {
    ...
    <div className="fixed w-full h-full z-50 bg-slate-200 bg-opacity-35 top-0 left-0 right-0 bottom-0 flex justify-center items-center">
      <div className="bg-white p-4 rounded w-[95%] max-w-2xl h-full max-h-[80%] overflow-hidden ">
        ...
        >
          ...
          <button className="px-3 py-2 rounded bg-red-600 text-white mb-10 hover:bg-red-700">
            Update Product
          </button>
        </form>
      </div>

      {/* Display Full uploaded Image */}
      {openEnlargeImage && (
        <EnlargeImage
          onClose={() => setOpenEnlargeImage(false)}
